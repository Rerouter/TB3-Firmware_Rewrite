...
case 52:  // loop for external interrupt - external triggering
    ...
    //New interrupt Flag Checks
      ...
      if( (Program_Engaged) && !(Shot_Sequence_Engaged) && !camera_shutter() && (ext_shutter_open) ) { //start a shot sequence flag
      ...
      if( (Program_Engaged) && (Shot_Sequence_Engaged) && !camera_shutter() && (ext_shutter_open) ) { //fire the camera can happen more than once in a shot sequence with HDR
       ...
       if (Shot_Sequence_Engaged && camera_shutter() && !(ext_shutter_open) ) { //shutter just closed, stop the camera port and move
...
case 250:  // loop for Pano
    ...
    //Kick off the shot sequence!!!  This happens once per camera shot.   
      ...
      //if (Program_Engaged && Shot_Sequence_Engaged && Static_Time_Engaged && !Shutter_Signal_Engaged && ((millis() - interval_tm) > (prefire_time*100+static_tm*100)) ) {
      if (Shot_Sequence_Engaged && Static_Time_Engaged && !camera_shutter() && ((millis() - interval_tm) > (prefire_time*100+static_tm*100)) ) { //removed requirement for Program Engaged for external interrupt
